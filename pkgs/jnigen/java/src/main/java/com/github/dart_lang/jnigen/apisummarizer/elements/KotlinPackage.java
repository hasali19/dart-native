...
public class KotlinPackage {
  ...
  public static KotlinPackage fromKmPackage(KmPackage p) {
    ...
    pkg.functions = p.getFunctions().stream().map(KotlinFunction::fromKmFunction).collect(Collectors.toList());
    pkg.properties = p.getProperties().stream().map(KotlinProperty::fromKmProperty).collect(Collectors.toList());
    return pkg;
  }
}
