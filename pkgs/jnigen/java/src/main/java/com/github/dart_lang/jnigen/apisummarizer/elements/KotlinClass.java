...
public class KotlinClass {
    ...
    public static KotlinClass fromKmClass(KmClass c) {
        ...
        klass.functions = c.getFunctions().stream().map(KotlinFunction::fromKmFunction).collect(Collectors.toList());
        klass.properties = c.getProperties().stream().map(KotlinProperty::fromKmProperty).collect(Collectors.toList());
        klass.constructors = c.getConstructors().stream()
                .map(KotlinConstructor::fromKmConstructor)
                .collect(Collectors.toList());
        klass.typeParameters = c.getTypeParameters().stream()
                .map(KotlinTypeParameter::fromKmTypeParameter)
                .collect(Collectors.toList());
        klass.contextReceiverTypes = c.getContextReceiverTypes().stream()
